...
function configureBot(bot) {
  ...
  /**
   * Main loop for a tree farming routine that will deliver wood to a player every 10 collected
   * @param deliveryThreshold
   */
  function treeFarmer(deliveryThreshold = 10, failureCount = 0) {
      ...
      if (farmingDeliveryRun) {
        ...
        if (target) {
          ...
          comeToPlayer(target[1].entity.username).then(() => {
            dropInventoryItem(target[1].entity.username, 'log').then( () => {
  ...
  function findAndAttackTarget(username, targetType) {
    ...
    const entity = bot.nearestEntity(ne => (!targetType || (ne.name && (ne.name.toLowerCase() === targetType.toLowerCase())) || (ne.displayName && (ne.displayName.toLowerCase() === targetType.toLowerCase()))) && (!username || ne.username !== username) && ne.isValid && (ne.health > 0) && (ne.type === 'mob' || ne.type === 'player'));
    if (!entity) {
